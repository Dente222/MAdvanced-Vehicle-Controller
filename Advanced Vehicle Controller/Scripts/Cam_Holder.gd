...
func _physics_process(delta: float) -> void:
	...
	if current_velocity.length_squared() > 1: # Keep at 1 to prevent camra from glithing when holding hand break
	...
	# We Restrict X rotation to 60+/- dgr to prevent camrea from flipping
